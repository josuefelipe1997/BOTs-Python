'''
Esse codigo verifica o status das chamadas no discador tristerix, e caso obtenha um status mais de 20 vezes consecutivas
ele envia uma mensagem no whatsapp informado relatando o erro e o horrário que ele foi obtido.
...
telefone = int(input('\nInforme o telefone que ira receber as notificações: '))
...
while True:
    ...
    try:  
        ...
        if count_atentida >= 20 or count_responde >= 20 or count_area >= 20 or count_canal >= 20:
            data = datetime.datetime.now()
            hora = str(data.hour) + ':' + str(data.minute) + ':' + str(data.second)
            kt.sendwhatmsg_instantly(telefone, 'SYSTEM INFORMATION: Mais de 10 {verificador_status} seguidos as {hora}')
            time.sleep(5)
            while True:
                try:
